...
<body class="bg-gradient-to-br from-eco-light to-green-50 min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
    <div class="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p class="mt-2 text-center text-sm text-gray-600">
            ...
            <a href="<?= URLROOT ?>/auth/login" class="font-medium text-eco-primary hover:text-green-500 transition duration-150 ease-in-out">
                Sign in here
            </a>
        </p>
    </div>
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-lg">
        <div class="bg-white py-8 px-4 shadow-2xl sm:rounded-xl sm:px-10 border border-gray-100 relative overflow-hidden">
            ...
            <form class="space-y-6 relative z-10" action="<?= URLROOT ?>/auth/register" method="POST">
                <div>
                    <label for="username" class="block text-sm font-medium text-gray-700">
                        Username
                    </label>
                    <div class="mt-1">
                        <input id="username" name="username" type="text" required value="<?= $data['username'] ?? '' ?>" class="appearance-none block w-full px-3 py-2 border <?= (!empty($data['username_err'])) ? 'border-red-500' : 'border-gray-300' ?> rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-eco-primary focus:border-eco-primary sm:text-sm transition duration-150 ease-in-out">
                        <span class="text-red-500 text-xs italic"><?= $data['username_err'] ?? '' ?></span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <input id="phone" name="phone" type="text" autocomplete="tel" value="<?= $data['phone'] ?? '' ?>" class="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-eco-primary focus:border-eco-primary sm:text-sm transition duration-150 ease-in-out">
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="mt-1 relative">
                            <input id="password" name="password" type="password" required class="appearance-none block w-full px-3 py-2 border <?= (!empty($data['password_err'])) ? 'border-red-500' : 'border-gray-300' ?> rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-eco-primary focus:border-eco-primary sm:text-sm transition duration-150 ease-in-out pr-10">
                    ...
                    <div>
                        ...
                        <div class="mt-1 relative">
                            <input id="confirm_password" name="confirm_password" type="password" required class="appearance-none block w-full px-3 py-2 border <?= (!empty($data['confirm_password_err'])) ? 'border-red-500' : 'border-gray-300' ?> rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-eco-primary focus:border-eco-primary sm:text-sm transition duration-150 ease-in-out pr-10">
                            <button type="button" onclick="togglePassword('confirm_password')" class="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-400 hover:text-gray-500 focus:outline-none">
                                <svg id="confirm_password_eye" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                </svg>
                            </button>
                        </div>
                        <span class="text-red-500 text-xs italic"><?= $data['confirm_password_err'] ?? '' ?></span>
                    </div>
                </div>
